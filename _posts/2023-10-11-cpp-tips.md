...
（2023-10-11）将此前的笔记更新到个人站上，此后会不定期更新。
...
![image-20220406165414337](/assets/2023-10-11-cpp-tips/image-20220406165414337.png)
...
![cpp-hell](/assets/2023-10-11-cpp-tips/image-20220530152626105.png)
...
- 初始化矩阵：我查了一下，是下面这张图（来自[这篇博客](https://timur.audio/initialisation-in-c17-the-matrix)）：
  ...
  ![cpp-initialization](/assets/2023-10-11-cpp-tips/init_in_cpp17_table.png)
...
- `range-for` 循环有缺陷：自从 **C++*11** 引入 `range-for` 循环之后，其因语法的简洁性被大量使用。下面是（截至 **C++20**）它的等价形式：
  ...
  （来自 2023 年的（不太新的）新消息：C++23 通过了对 `range-for` 的语义更新，现在里面初始化的变量的生命周期延长到整个语句块了，可喜可贺！）
...
## SFINAE 的使用误区（2023/10/15）

在我的脑海中，下面这串代码是可以通过编译的：
...
不过显然我没有测试过。实际上支持 SFINAE 的环境只有函数签名、模版签名中出现的表达式或类型（查了一下甚至还有一个 `explicit` 关键字），因此上面这串代码需要进行修改。我的第一个反应是进行函数重载：
...
很可惜这也不可行：这并没有改变编译错误的本质：`requires` 表达式并非可以出现 SFINAE 的场景；我们需要将 SFINAE 控制在 `template<typename T>` 或 `void foo(T&& t)` 这两个范围当中。下面是一个典型的 `std::void_t` SFINAE 示例：
...
```
现在这个实现就没有任何问题了。对比最早我“幻想中”的实现方案不得不说复杂了很多。

但是！故事到这里没有结束。我的实际代码中逻辑是要比上面的例子复杂的（这也是连带出下面问题的原因），我在切换到 `std::void_t<>` 之后依然会出现实例化错误。经过一些研究后我意识到一点：SFINAE 只能发生于*当前正在实例化的模版*。下面是一个简化过的说明代码：
...
template<typename T>
concept has_traits = requires {
    typename T::type;
};

template<typename T>
void foo(T&& t) {
    if constexpr (has_traits<T>) {
        // 省略内容
    }
    else {
        // 省略内容
...
上面是我最开始的代码，可以看到这里我犯的错误和本小节开始时一样，也就是在错误的地方使用 SFINAE（虽然！往下看就会发现这甚至不是一个 SFINAE），因此我修改成了 `std::void_t<>` 的版本，但报错依然存在。真正的问题在于：C++ 不允许 SFINAE 出现在模版声明的“深处”。看下面的例子：

```cpp
template<typename T>
struct sfinae_friendly<T, std::void_t<typename T::type>> {};

template<typename T>
struct sfinae_unfriendly<T, std::void_t<typename whatever_traits<T>::type>> {};
```

这里 `sfinae_friendly` 中所有可能出现的替换失败都出现在 `typename T::type` 这里（也就是 `T` 中没有定义 `type`），因此 SFINAE 可以奏效；但 `sfinae_unfriendly` 中我们使用了 `whatever_traits`，如果它的实例化失败，则*不会*被编译器认为是 `sfinae_unfriendly` 的替换失败，而是一个编译错误（做得好哇 C++）。如此一来，我们就需要尽可能避免替换失败的意外发生了。对于我这个特定的例子，应该让 `my_traits` 加上一个 SFINAE：
...
template<typename T>
struct my_traits<T, std::void_t<typename T::type>> {
    using type = typename T::type;
};
```

这样调用 `typename my_traits<T>::type` 就不会出错了；至于判断是否 `has_traits`，就需要进行特定的比较：

```cpp
template<typename T>
concept has_traits = !std::is_same_v<typename my_traits<T>::type, void*>;
```

这种比较在逻辑上是有风险的，因为我们无法保证用户在偏特化 `my_traits` 时将 `type` 定义为 `void*`（或其它你设置的默认类型）。原本可以用“空”来代表的无效状态，现在就得占用一个类型来设置。显然不能指望标准为此作出改变，但 C++ TMP 真的太啰嗦了，很期待静态反射能早日 ship。